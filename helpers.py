...
def board_texture_description(board_codes):

    #need at least 3 cards to evaluate texture
    if len(board_codes) < 3:
        return "No flop yet"
    
    #ranks anb suits
    ranks = [c[0] for c in board_codes]
    suits = [c[1] for c in board_codes]
    ...
    #flush draw if 2 cards of same suit    
    flush_d = max(suit_counts.values()) >= 2

    rank_vals = sorted(RANK_ORDER.index(r) for r in ranks)

    #straight draw if cards are close in rank
    straight_d = (max(rank_vals) - min(rank_vals) <= 4) and len(set(ranks)) >= 3

    #possibility of two cards sharing same rank
    paired_board = len(set(ranks)) < len(ranks)

    if flush_d and straight_d:
        return "Wet board"
    elif flush_d and not straight_d:
        return "Semi-wet board (flush draw possible)"
    elif straight_d and not flush_d:
        return "Semi-wet board (straight draw possible)"
    elif paired_board and not flush_d and not straight_d:
        return "Paired dry board"
    else:
        return "Dry board"
